...
<html>
	<head>
		...
		//str is passed to the function by the onkeyup event
			...
			{
				//if nothing is typed, nothing isreturned
				if(str.length==0)
				{
					document.getElementById("txtHint").innerHTML="";	
					return;
				}
		//the if-else construct checks to see if the browser is older. Most
		//modern browsers can create an XMLHttpRequest() object but older
		//browsers use an ActiveXObject(). Either object establishes an independent
		//connection channed between our client-side web page and the server-side 
		//php script
		...
		//The onreadystatechange event is triggered every time the readyState changes.
				...
				{
		//checks if readyState is 4(request is finished and response is ready) and status is 200
		//(page is found). If TRUE then response is ready.
					if(xmlhttp.readyState==4 && xmlhttp.status==200)
					{
		//The 'responseText' property is used to retrieve the server's response as a string
			document.getElementById("txtHint").innerHTML = xmlhttp.responseText;	
					}
				}
		//open(method, url, async) has 3 arguments. Here the method=GET. The second argument is 
		//the location of server-side processing- gethint.php with an appended parameter, q, equal
		//to str which holds whatever was typed into the input box. The asynch argument is set to 
		//TRUE so that server-side script processing continues after the send() method, without
